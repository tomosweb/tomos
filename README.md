...
現在のバージョンは `v1.0.0` です。これはv1.0.0 Release Candidateです。
...
>
> 2まで完了するとalpha.18の新しいUpdaterへ完全に移行します。その後、Tomos Updateからalpha.19、beta.1へ1バージョンずつ更新できます。現在のv1.0.0 RCへの更新は、下記のv1.0.0更新案内を確認してください。

詳しくは [v1.0.0の更新案内](https://github.com/tomosweb/tomos-dev/blob/release/v1.0.0-rc/docs/releases/v1.0.0.md) を確認してください。過去のalpha/betaからの移行手順は[公式サイトの更新手順](https://tomoswords.org/docs/update/)に掲載しています。
...
外部Markdown投稿を利用する場合は、Tomos Postの「セキュリティ」から投稿用トークンを発行し、HTTPS投稿APIへ送信します。投稿用トークンは管理用合言葉とは別で、再発行すると以前のトークンは無効になります。トークンは発行時に表示される値を安全に保管してください。画像付き投稿はJPEG、PNG、GIF、WebPに対応し、1記事5点まで、1点10MBまでです。自動公開に失敗した投稿は、Tomos Postの「下書き」から確認・公開・削除できます。
