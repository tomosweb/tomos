...
`v0.1.0-alpha.2` では、フォルダー記事一覧のページングを追加し、数字だけの第1階層フォルダー名で発生するPHP 8の型エラー、サブディレクトリ設置時にTomos Postの公開URLへ設置パスが二重に付く問題、日本語の濁点表現だけがNFC/NFD形式で異なる同名ファイルを更新確認なしで二重公開する問題を修正しました。
