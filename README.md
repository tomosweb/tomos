...
現在のバージョンは `v0.1.0-alpha.1` です。限定テスト用の初期開発版であり、正式リリース版ではありません。
...
`v0.1.0-alpha.1` では、Tomos Postで既存ページの更新確認直後に連続投稿制限がかかる問題を修正しました。

既存環境では、更新前にバックアップを取り、配布Zipを別フォルダへ展開して、次の2ファイルだけを同じパスへ上書きします。

- `core/PostRateLimiter.php`
- `post/index.php`

`config.php`、`content/`、`themes/`、`cache/`、`trash/` は上書きまたは削除しないでください。配布Zip全体を既存の設置ルートへそのまま上書き展開しないでください。
