...
try {
    ...
    $pointer = json_decode((string) file_get_contents($candidate . '/latest.json'), true);
    check(is_array($manifest), 'candidate manifest is JSON');
    check(is_array($pointer), 'candidate pointer is JSON');
    check(($pointer['schema_version'] ?? null) === 1, 'pointer schema version is supported');
    ...
    check(strpos($pointer['manifest_url'], '/installer/releases/' . $version . '/install-manifest.json') !== false, 'pointer uses mirror manifest URL');
    check(strpos($pointer['signature_url'], '/installer/releases/' . $version . '/install-manifest.sig') !== false, 'pointer uses mirror signature URL');
    check(strpos($manifest['asset']['url'], '/installer/releases/' . $version . '/tomos-' . $version . '.zip') !== false, 'manifest uses mirror ZIP URL');
    $installerSource = (string) file_get_contents($candidate . '/install.php');
    check(strpos($installerSource, 'require_once') === false, 'installer has no external require');
    check(strpos($installerSource, 'fixture.test') === false, 'installer has no fixture URL');
    check(strpos($installerSource, '-----BEGIN PRIVATE KEY-----') === false, 'installer has no private key');
    check(strpos($installerSource, 'https://tomoswords.org/installer/latest.json') !== false, 'installer uses production mirror pointer');
    check(strpos($installerSource, 'https://tomoswords.org/download/install/latest.json') === false, 'installer excludes legacy production pointer');
    check(strpos($installerSource, "'/installer/releases/' . \$version") !== false, 'installer accepts mirror release paths');
