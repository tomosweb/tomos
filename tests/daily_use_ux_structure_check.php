...
assertContains($index, 'M16 3c-2.8 3.5-4.2 6.1-4.2 8.5', 'Daily message Tomos mark must include the separated official-style flame');
assertContains($index, 'fill-rule="evenodd"', 'Daily message flame must retain its internal cutout');
assertContains($index, 'stroke="#19344d"', 'Daily message candle must use the official dark navy family');
assertContains($index, 'stroke-width="2.25"', 'Daily message candle body must use a restrained line weight');
assertContains($index, 'M22 25v12', 'Daily message candle must retain the second downward body structure');
...
assertNotContains($messageFunction, 'fill="#2c3e50"', 'Daily message mark must not use the older heavy navy fill treatment');
assertNotContains($messageFunction, 'M16 18v-4', 'Daily message mark must not use the rejected standalone wick treatment');
assertNotContains($messageFunction, '<circle', 'Daily message mark must not include the official outer circle');
assertNotContains($messageFunction, '<ellipse', 'Daily message mark must not include a background ellipse');
