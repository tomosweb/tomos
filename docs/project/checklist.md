...
- [ ] 記事Aの直後に異なる `submission_id` で記事Bを投稿できる
- [ ] 同じ `submission_id` の最終保存を再送しても二重保存されない
- [ ] 同じ `submission_id` は完了から10分経過後も24時間以内は再送を拒否される
- [ ] 完了から24時間経過後に投稿完了記録とロックファイルが整理される
- [ ] 投稿失敗後は同じ `submission_id` で再試行できる
- [ ] 画像の分割送信が投稿間隔制限を受けず、同じ `submission_id` に紐づく
- [ ] 記憶を選ばない認証はPHPセッション中だけ維持される
- [ ] 記憶を選んだ認証は30日間有効で、改ざん・期限切れ・サーバー記録欠落時は認証されない
- [ ] 認証解除でCookie、PHPセッション、対応するサーバー記録が削除される
- [ ] 合言葉再発行後は既存の記憶Cookieで認証されない
- [ ] `/post/reset/` の短時間連続POSTが拒否される
- [ ] `cache/security/post-auth/` と `cache/security/post-submissions/` が直接閲覧できない
- [ ] 記憶トークン、投稿完了記録、ロックファイルが配布ZIPに含まれない
...
- [ ] 誤った合言葉では取り下げできない
- [ ] 正しい合言葉で取り下げできる
- [ ] 合言葉失敗による15分停止中は未認証の取り下げ操作を実行できない
