...
設置後にテーマだけを変更する場合、`setup/` は使いません。`/post/` のサイト設定からテーマ管理へ進みます。テーマ変更にも管理用合言葉を使います。
...
設置後はTomos Postのテーマ管理画面からテーマZIPを追加できます。追加したテーマは自動では有効にならず、テーマ一覧から明示的に切り替えます。
...
Tomos Postは、Tomos Writeなどで作成したMarkdownファイルを `/post/` から投稿するための機能です。
...
Tomos Postを有効にした場合は、完了画面に表示される管理用合言葉を控えてください。

合言葉を忘れた場合、登録済みパスキーがあればTomos Postの「セキュリティ」から合言葉を再設定できます。パスキー未登録で利用条件を満たす環境では、サーバー所有確認による復旧経路を利用できます。これらを利用できない場合は、legacy緊急手段として `post-reset.enable` と `/post/reset/` を使用できます。再発行後は `post-reset.enable` を削除してください。

安全のため、セットアップ完了後は `setup/` ディレクトリを削除してください。

テーマだけを変更したい場合は、`setup/` を再有効化せず、Tomos Postのテーマ管理を使います。テーマZIPの追加と使用テーマの切り替えはsetup後に行えます。
