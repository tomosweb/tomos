...
各項目の`path`はTomos内部の絶対パスで、外部URL、query、fragment、traversalは無効として無視されます。`label`は任意で、空または省略時はauto navigationがその宛先に使うラベルを採用します。`hidden`は主要ナビゲーションから隠すだけで、対象ページのrouting・公開状態・直接URLへの到達性は変更しません。
