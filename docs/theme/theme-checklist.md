...
- [ ] 検索ページが読める
- [ ] 画像が本文幅からはみ出さない
- [ ] YouTube動画（`.youtube-embed`）が本文幅からはみ出さず、PC・スマホとも16:9で表示される
...
- [ ] 未認証で `/post/theme/` にアクセスすると、認証要求を表示したTomos Postへ移動する
- [ ] 認証成功後に `/post/theme/` へ戻る
- [ ] 不正な `return_to` はTomos内部の設定画面へfallbackし、外部URLへ遷移しない
- [ ] 有効テーマだけ選択できる
- [ ] 無効テーマは選択できない
- [ ] `theme.name` だけが変更される
