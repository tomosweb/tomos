...
GDが使えない場合、画像加工用メモリの安全判定を満たさない場合、または画像加工中に例外が発生した場合は、安全側へフォールバックして対応形式の画像を元のまま保存します。元画像の保存に成功した場合、これらの内部的なフォールバック理由は利用者向け警告として表示せず、診断用にPHPエラーログへ記録します。元画像の保存自体に失敗した場合はエラーとして扱います。公開HTMLのローカル画像URLには更新識別子を付けるため、同じ管理名の画像を縮小版へ置き換えた場合も古いキャッシュを参照し続けません。
