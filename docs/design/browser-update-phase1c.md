...
ブラウザ更新の逐次性は、catalogの`from/to`と署名済みmanifestの結果を両方照合して保証します。`summary['current_version']`が期待するfrom、`summary['version']`が期待するtoと一致しなければ`update_sequence`で拒否します。catalogは次に取得すべきパッケージの案内であり、Update ZIPの真正性の最終根拠ではありません。最終的な信頼判定は既存の`update/public-key.pem`による`manifest.sig`検証です。
