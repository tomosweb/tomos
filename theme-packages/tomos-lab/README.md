...
Navigation Settings v1の動作確認では、`sample-site/theme-settings.php`の`navigation`設定を使います。manual modeの並び順、label上書き、label省略時の自動ラベル、`hidden`によるナビゲーション非表示と、非表示ページの直接到達性を確認します。
