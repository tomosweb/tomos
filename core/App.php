...
final class App
{
    ...
    private function sendSecurityHeaders(): void
    {
        ...
        if (!empty($this->config['security']['content_security_policy'])) {
            $allowThemeScripts = !empty($this->config['security']['allow_theme_scripts']);
            if (Ga4::measurementId($this->config) !== '' && $this->ga4Nonce !== '') {
                $scriptSource = $allowThemeScripts ? "'self' https://www.googletagmanager.com 'nonce-" . $this->ga4Nonce . "'" : "https://www.googletagmanager.com 'nonce-" . $this->ga4Nonce . "'";
                header("Content-Security-Policy: default-src 'self'; script-src " . $scriptSource . "; connect-src 'self' https://www.google-analytics.com https://*.google-analytics.com https://*.analytics.google.com; style-src 'self'; img-src 'self' data: http: https:; frame-src https://www.youtube.com; object-src 'none'; base-uri 'self'; frame-ancestors 'none'");
            } elseif ($allowThemeScripts) {
                header("Content-Security-Policy: default-src 'self'; script-src 'self'; connect-src 'self'; style-src 'self'; img-src 'self' data: http: https:; frame-src https://www.youtube.com; object-src 'none'; base-uri 'self'; frame-ancestors 'none'");
            } else {
