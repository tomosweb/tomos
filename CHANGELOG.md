...
- Tomos Postから外部テーマZIPをアップロードし、検査結果を確認して追加できるようにしました。
- 追加したテーマを既存のテーマ管理画面から選択し、公開サイトへ反映できるようにしました。

### 改善

- Finderで作成したZIPに含まれるmacOSメタデータを安全に無視するようにしました。
- テーマの必須ファイル5点と推奨ファイル3点を区別して検査するようにしました。
- 既存テーマと同じIDのテーマZIPによる上書きを禁止しました。

### セキュリティ

- 絶対パス、親ディレクトリ参照、重複パス、シンボリックリンク、特殊ファイルを含むテーマZIPを拒否します。
- PHPファイルおよびJavaScriptファイルを含むテーマZIPを拒否します。
- 一時領域での検査、制御された展開、ロック、隠しステージング、失敗時の復元を使用してテーマを追加します。
