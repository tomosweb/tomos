...
インストール対象バージョン: `v0.1.0-alpha.6`

> この手順は `v0.1.0-alpha.6` の限定テスト版を設置するためのものです。
...
1. 配布された `tomos-0.1.0-alpha.6.zip` を受け取ります。
...
設置後にテーマだけを変更したい場合は、`/post/` のサイト設定からテーマを切り替えます。テーマ変更にも管理用合言葉を使います。
...
初期セットアップ完了後にサイト名やサイトの説明文などを変更する場合は、Tomosを設置したディレクトリにある
`config.php` を編集します。
